param(
    ...
    [ValidateSet("singleEc2Instance")][string] $resourceType,
    [string] $sshUser = "ubuntu"
)

if ($resourceType -eq "singleEc2Instance") {

    $instanceId = (Get-CFNStackResource -Region $region -AccessKey $awsAccessKey -SecretAccessKey $awsSecretKey -StackName $stackName -LogicalResourceId $LogicalResourceId).PhysicalResourceId
    $publicIpAddress = (get-ec2instance -InstanceId $instanceId -Region $region -AccessKey $awsAccessKey -SecretKey $awsSecretKey | Select-Object -ExpandProperty Instances).PublicIpAddress
...
$puttyReg = 'Windows Registry Editor Version 5.00

[HKEY_CURRENT_USER\Software\SimonTatham\PuTTY\Sessions\KEY:%20eddy-scratch@intapp.com_ubuntu]
...
"PublicKeyFile"="C:\\Users\\eddy\\Downloads\\eddy-scratch@intapp.com.ppk"
"RemoteCommand"=""
...
$hostName = $($sshUser + "@" + $publicIpAddress)

$newPuttyReg = $puttyReg.Replace("NEW_HOSTNAME_HERE", $hostName)
$newPuttyReg | Out-File $("$env:APPDATA\puttySession.reg")

try {

    REG IMPORT $( "$env:APPDATA\puttySession.reg" ) | Out-Null
...
putty -load "KEY: eddy-scratch@intapp.com_ubuntu"